  ...
  methods: {
    ...
    initOption() {
      ...
      this.option = {
        ...
        series: [
          {
            top: this.title ? this.transformFontSize(40) : "",
            name: "",
            radius: this.radius || ["55%", "72%"],
            type: "pie",
            startAngle: 45,
            minAngle: 10,
            label: {
              overflow: "none",
              formatter: (data) => {
                return `{name|${data.name}}\n{time|${data.value}${this.unit ||
                  "个"}} {time|${data.percent}%}`;
              },
              lineHeight: this.transformFontSize((this.pieData || []).length >= 4? 18:22),
              rich: {
                name: {
                  color: (this.pieData || []).length >= 5 ? null : "#fff",
                  fontWeight: "400",
                  fontSize: this.transformFontSize(
                    (this.pieData || []).length >= 4 ? 14 : 18
                  ),
                  align: "center",
                },
                time: {
            ...
            labelLayout: (params) => {
              ...
              console.log("isLeft", isLeft, params);
              points[2][0] = isLeft
                ? params.labelRect.x
            ...
            data:
              (this.pieData || []).length >= 5
                ? (this.pieData || []).map((e, index) => {
                    e.label = {
                      color:
                        colorList[index % colorList.length].colorStops[0]
                          .color || "#fff",
                    };
                    return e;
                  })
